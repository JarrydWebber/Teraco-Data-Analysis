...
def nnls_regression(Y_data, X_data, Timeframe, Aggregation_Period, Aggregation_method="mean"):
    ...
    if Aggregation_method == "mean":
        x_reg = X_data.resample(Aggregation_Period).mean()
        y_reg = Y_data.resample(Aggregation_Period).mean()
        x_for_pred = raw_X_data.resample(Aggregation_Period).mean()

    else:
        x_reg = X_data.resample(Aggregation_Period).sum()
        y_reg = Y_data.resample(Aggregation_Period).sum()
        x_for_pred = raw_X_data.resample(Aggregation_Period).sum()
    ...
    if inf + nans > 0:
        ...
        x_reg.interpolate(method="backfill", inplace=True)
    x_reg = sm.add_constant(x_reg)
    x_for_pred = sm.add_constant(x_for_pred)

    columns_used = x_reg.columns
    results = nnls(x_reg, y_reg)[0]

    x_for_pred["Pred"] = 0
    x_reg["Pred"] = 0
    # print(x_reg.columns, results)
    for factor, coeff in zip(x_reg.columns, results):
        # print (factor, coeff, x_reg[factor].values)
        x_for_pred["Pred"] += coeff * x_for_pred[factor].values
        x_reg["Pred"] += coeff * x_reg[factor].values

    r2 = r2_score(y_reg, x_reg["Pred"])

    return ([results, x_for_pred["Pred"], r2, columns_used])
...
def parameterisation(Y_data, X_data, Timeframe, Aggregation_Period, Aggregation_method="mean", target_R=0.1, overfitting=True):
    ...
    for column in X_data.columns:
        ...
        current += 1
        result = nnls_regression(Y_data, X_data[column], Timeframe, Aggregation_Period, Aggregation_method)
        # print (column, result.rsquared_adj)
        coefficients, predicted_Y_nnls, r2_nnls, cols = result
        if r2_nnls >= target_R:
...
#        print (data[names])
            ...
            grouped_frame["{0} {1}".format(tem, t)] = data[names].sum(axis=1)
            diffed_data[names] = data[names].fillna(0)

diffed_data["const"] = diffed_data[diffed_data.columns[0]] * 0 + 1

# ### Weather Import
raw_start = diffed_data.index[0]
raw_end = diffed_data.index[-1]
file_name = r"weather.csv"
weather = pd.read_csv(file_name, skiprows = 6, parse_dates = True, index_col = "Week starting")
weather.index = weather.index + datetime.timedelta(days = 7)
diffed_data = pd.concat([diffed_data, weather], axis = 1)[raw_start:raw_end]

inf = np.isinf(diffed_data).sum().sum()
nans = diffed_data.isna().sum().sum()
if inf + nans > 0:
    # print(inf, nans)
    diffed_data.interpolate(method="backfill", inplace=True)

inf = np.isinf(diffed_data).sum().sum()
nans = diffed_data.isna().sum().sum()
if inf + nans > 0:
    # print(inf, nans)
    diffed_data.fillna(0, inplace=True)
# ### Weather Import End
...
UPS_data_excel = pd.DataFrame()
Incoming_data_excel = pd.DataFrame()
...
for tem in TEMS_list:
    if "{0} HVAC".format(tem) in grouped_frame.columns:
        ...
        for period in period_list:
            ...
            if tem != "Total":
                diffed_data["Total UPS {0}".format(tem)] = diffed_data[nameing_list[(nameing_list["Type"] == "UPS") & (
                                                                        nameing_list["Energy Flow"] == "Outgoing") & (
                                                                        nameing_list["TEM"] == tem)][
                                                                    "Full Name"]].sum(axis=1)
            else:
                diffed_data["Total UPS {0}".format(tem)] = diffed_data[nameing_list[(nameing_list["Type"] == "UPS") & (
                                                                        nameing_list["Energy Flow"] == "Outgoing")][
                                                                    "Full Name"]].sum(axis=1)
            # print (diffed_data["Total UPS {0}".format(tem)].to_frame())

            print(diffed_data.columns)

            important_params, perms_list = parameterisation(target,
                                                            diffed_data[["Total UPS {0}".format(tem), "CDD 18"]],
            ...
            for pair in perms_list:
                ...
                regression_result = nnls_regression(target, predictors, [baseline_date1, baseline_date2], period)
                # print(regression_result.rsquared_adj)

                if round(regression_result[2], accuracy) > best_R:
                    best_R = regression_result[2]
                    best_params = pair
                    best_period = period
        ...
        predicted_Y = best_results[1]#.predict(diffed_data[best_results.params.index].resample(best_period).mean())
        ...
        try:
            ...
            tem_frame_excel_target.columns = ["R2 = " + str(targeting_regression_result.rsquared_adj)]
        ...
        temp_UPS_data = diffed_data[nameing_list[(nameing_list["Type"] == "UPS") & (nameing_list["TEM"] == tem)]["Full Name"]].sum(axis = 1).resample(best_period).mean().reset_index()
        temp_UPS_data.columns = ["Timestamp", "Total UPS Power"]
        temp_UPS_data["TEM"] = tem

        temp_Incoming_data = diffed_data[nameing_list[(nameing_list["Energy Flow"] == "Incoming") & (nameing_list["TEM"] == tem)]["Full Name"]].sum(axis=1).resample(best_period).mean().reset_index()
        temp_Incoming_data.columns = ["Timestamp", "Total Incoming Power"]
        temp_Incoming_data["TEM"] = tem

        if tem == "Total":
            temp_UPS_data = diffed_data[
                nameing_list[(nameing_list["Type"] == "UPS")]["Full Name"]].sum(
                axis=1).resample(best_period).mean().reset_index()
            temp_UPS_data.columns = ["Timestamp", "Total UPS Power"]
            temp_UPS_data["TEM"] = tem

            temp_Incoming_data = diffed_data[
                nameing_list[(nameing_list["Energy Flow"] == "Incoming")][
                    "Full Name"]].sum(axis=1).resample(best_period).mean().reset_index()
            temp_Incoming_data.columns = ["Timestamp", "Total Incoming Power"]
            temp_Incoming_data["TEM"] = tem

        UPS_data_excel = pd.concat([UPS_data_excel, temp_UPS_data])
        Incoming_data_excel = pd.concat([Incoming_data_excel, temp_Incoming_data])
        ...
        plt.close()
        # print (best_results)
        tem_frame_excel = pd.DataFrame(data=best_results[0])#.params)
        tem_frame_excel.index = best_results[3]
        tem_frame_excel.columns = ["R2 = " + str(best_results[2])]#.rsquared_adj)]
...
grouped_frames = [target_data_excel, predicted_data_excel, kept_target_data_excel,
                                 ...
                                 target_pue_data_excel, UPS_data_excel, Incoming_data_excel]
...
for tem in tems_used:
    ...
    for frame in grouped_frames:
        if "TEM" in frame.columns:
            # print(tem, tems_used, frame.columns)
    ...
    tem_frame_grouped["CDD 18"] = diffed_data["CDD 18"]
...
if testing_graphs_print:
    ...
    for full, name in zip(nameing_list[nameing_list["Energy Flow"] == "Incoming"]["Full Name"],
        ...
        plt.plot(diffed_data[full][date1:date2], label=[name])
    plt.legend()
